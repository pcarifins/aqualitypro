const fs = require('fs');
let code = fs.readFileSync('src/components/PriorityQueue.tsx.bak', 'utf-8');

const returnStartMatch = '  return (\n    <div className="space-y-5 pb-24 max-w-6xl mx-auto px-2 sm:px-4 pt-3">';
const componentReturnIndex = code.indexOf(returnStartMatch); 
if (componentReturnIndex === -1) {
    console.error("Could not find start match!");
    process.exit(1);
}
const preReturn = code.substring(0, componentReturnIndex);
